...
import { CafeEmployeeState, Cafe } from "../redux/types";
import {
  getCafesAction,
  setLocationAction,
...
const CafePage = () => {
  ...
  const fieldName = useCallback((name: keyof Cafe) => name, []);

  const cafes = useSelector((state: CafeEmployeeState) => state.cafes);

  const pinnedBottomRowData = useMemo(
    () => createPinnedBottomAddButtonRow("Add New Cafe", true, "/add"),
    []
  );
  ...
  const colDefs: ColDef[] = [
    ...
    {
      field: fieldName("logo"),
      cellRenderer: (params: any) => {
        const fileType = params.value.split(".");
        return (
          <Image
            width={60}
            height={60}
    ...
    { field: fieldName("location"), filter: CustomLocationFilter },
    { field: fieldName("employees") },
    {
  ...
  const handleFilter = () => {
    const filter = gridRef.current?.api.getFilterModel().location;
    if (filter !== undefined) {
      if (filter.value !== "") {
        dispatch(getCafeByLocationAction(filter.value));
      } else {
        dispatch(getCafesAction());
      }
    }
  ...
  const handleDoubleClick = (e: any) => {
    navigate("/employee", { state: { cafe: e.data.name } });
  };
